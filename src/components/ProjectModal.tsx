...
export function ProjectModal({ isOpen, onClose, project }: ProjectModalProps) {
  // Usar -1 para el tab "Información" en móvil, 0 para la primera imagen
  ...
  const nextSection = () => {
    setCurrentSection((prev) => {
      if (prev === -1) return 0;
      return (prev + 1) % projectSections.length;
    });
  };

  const prevSection = () => {
    setCurrentSection((prev) => {
      if (prev === 0) return projectSections.length - 1;
      if (prev === -1) return projectSections.length - 1;
      return prev - 1;
    });
  };

  const currentSectionData = currentSection >= 0 ? projectSections[currentSection] : null;
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="!max-w-[98vw] !w-[98vw] h-[90vh] p-0 overflow-hidden sm:!max-w-[98vw]">
        ...
        <div className="h-full max-h-full overflow-hidden">
          ...
          {/* DISEÑO MÓVIL (< lg) - Layout vertical con tabs */}
          <div className="lg:hidden h-full flex flex-col">
            
            {/* Header Compacto Móvil */}
            <div className="p-4 border-b bg-white flex-shrink-0">
              <Badge className="mb-2 bg-emerald-600 text-white text-xs">
                {project.category}
              </Badge>
              <h2 className="text-xl text-slate-900 mb-2">{project.title}</h2>
              ...
              <div className="flex flex-wrap gap-1">
                {project.technologies.slice(0, 4).map((tech) => (
                  <Badge key={tech} variant="secondary" className="bg-blue-100 text-blue-800 text-xs px-2 py-1">
                    {tech}
                  </Badge>
                ))}
              ...
              {currentSection === -1 && (
                <ScrollArea className="h-full">
                  <div className="p-4 space-y-6">
                    ...
                    {/* Main Features */}
                    <div>
                      <h3 className="text-lg text-slate-900 mb-3">Características Principales</h3>
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border">
                      <h3 className="text-lg text-slate-900 mb-3">Detalles Técnicos</h3>
                      <div className="space-y-3">
                        <div>
                          <h4 className="text-sm text-slate-900 mb-1">Arquitectura</h4>
                          <p className="text-slate-600 text-xs">
                            Aplicación web responsive desarrollada con React y componentes modulares.
                          </p>
                        </div>
                        <div>
                          <h4 className="text-sm text-slate-900 mb-1">Funcionalidades</h4>
                          <p className="text-slate-600 text-xs">
                            Sistema de gestión integral con interfaz intuitiva y reportes automáticos.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-4 rounded-xl text-white">
                      <h3 className="text-lg mb-2">¿Interesado en un proyecto similar?</h3>
                      ...
                      {project.title === 'EventoPro' ? (
                        <Button
                          ...
                          className="bg-white text-blue-600 hover:bg-blue-50 px-4 py-2 text-sm"
                        >
              ...
              {currentSection >= 0 && currentSectionData && (
                <div className="h-full flex flex-col">
                  ...
                  <div className="flex-1 min-h-0 bg-slate-50 p-4">
                    <div className="h-full bg-white rounded-lg shadow-sm flex items-center justify-center relative overflow-hidden">
                      ...
                      <Button
                        variant="outline"
                        onClick={prevSection}
                        className="absolute left-2 top-1/2 -translate-y-1/2 bg-white/95 backdrop-blur-sm hover:bg-white shadow-lg h-8 w-8 p-0"
                      >
                      ...
                      <Button
                        variant="outline"
                        onClick={nextSection}
                        className="absolute right-2 top-1/2 -translate-y-1/2 bg-white/95 backdrop-blur-sm hover:bg-white shadow-lg h-8 w-8 p-0"
                      >
                  ...
                  <div className="p-4 bg-white border-t max-h-48 overflow-y-auto">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <h3 className="text-lg text-slate-900">{currentSectionData.title}</h3>
                    </div>
                    ...
                    <div className="space-y-2">
                      {currentSectionData.features.slice(0, 3).map((feature, index) => (
                        <div key={index} className="flex items-start gap-2">
                          <div className="w-1.5 h-1.5 bg-blue-500 rounded-full mt-2 flex-shrink-0"></div>
                          <span className="text-slate-600 text-xs">{feature}</span>
                        </div>
                      ))}
                      {currentSectionData.features.length > 3 && (
                        <div className="text-xs text-slate-500 mt-2">
                          +{currentSectionData.features.length - 3} características más
                        </div>
                      )}
                    </div>
                  </div>
          ...
          {/* DISEÑO ESCRITORIO (≥ lg) - Layout de dos columnas original */}
          <div className="hidden lg:grid lg:grid-cols-2 h-full max-h-full overflow-hidden">
